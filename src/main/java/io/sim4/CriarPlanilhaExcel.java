...
public class CriarPlanilhaExcel {
    ...
    public void updateSheetCar(DrivingData data, String nomeDoArquivo, String nomeDaAba) {
        ...
        try (FileInputStream inputStream = new FileInputStream(nomeDoArquivo);
            ...
            newRow.createCell(1).setCellValue(data.getAutoID()); //Pegar o ID do carro
            ...
            newRow.createCell(8).setCellValue(data.getX_Position()); // Corrigido para pegar a posição X
            newRow.createCell(9).setCellValue(data.getY_Position()); // Corrigido para pegar a posição Y
